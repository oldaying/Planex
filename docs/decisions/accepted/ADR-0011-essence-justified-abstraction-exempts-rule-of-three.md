...
Formalizes the rebuttal to the strongest external critique of Planex's abstraction route. The rebuttal was implicit in [`why-four-abstractions.md`](../../concepts/canonical/why-four-abstractions.md) and was identified as a known gap (Gap 2) in the comparative study [`docs/research/2025-08-28-abstraction-as-form-comparative-study.md`](../../research/2025-08-28-abstraction-as-form-comparative-study.md). This ADR is the institutional record so that future contributors, reviewers, and external skeptics can be pointed to one place when the Rule-of-Three critique is raised.
...
**Net assessment:** Compresses. The contributor's one-time cost of learning the distinction (perhaps an hour of reading this ADR plus [`abstraction-form.md`](../../concepts/canonical/abstraction-form.md) plus [`alternative-perspectives.md`](../../concepts/background/alternative-perspectives.md)) is less than the recurring cost of defending each abstraction from Rule-of-Three critiques in every external review and every internal code review for the lifetime of the project.

### Q3. Is there a gap between the claim and the implementation?

- **Claim (in [`why-four-abstractions.md`](../../concepts/canonical/why-four-abstractions.md), ADR-0007, ADR-0008, ADR-0010):** the 5 shipping abstractions are essence-justified, grounded in the 6-tradition literature survey.
...
- Protects v0.4 and v4 from being dismissed as premature architecture. The five shipping abstractions each have documented tradition citations in [`why-four-abstractions.md`](../../concepts/canonical/why-four-abstractions.md); the three v4 proposals each have documented tradition citations in [`essence-derivation-v4-clean.md`](../../concepts/history/essence-derivation-v4-clean.md); the criterion is now uniform across both.
- Establishes a falsifiability hook (per Prerequisite 3 of [`abstraction-form.md`](../../concepts/canonical/abstraction-form.md)): if a future audit finds that an abstraction admitted as essence-justified does not in fact meet the three criteria, the admission ADR can be superseded and the abstraction retired. The criterion is the test.
...
This ADR only rebuts the Rule-of-Three critique. It does NOT cover:

- Whether the 5 shipping + 3 v4-proposed abstractions are **optimal** — this ADR defends them against one specific critique (Rule of Three). It does not argue that no better abstraction set exists. The conditional thesis in `abstraction-form.md` (3 prerequisites) is the stronger claim; this ADR is its defense at one front.
- The CI-automation gap for the essence-justified criterion — the ADR's own Q3 self-acknowledges that the criterion is "documented but not enforced". A future ADR-0014 may close this by adding a CI lint that flags any new abstraction whose constructor accepts a parameter no operation reads (i.e., abstraction-eroding L2 leak at the API surface). This is out of scope here.
