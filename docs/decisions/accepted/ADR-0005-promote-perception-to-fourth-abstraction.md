...
Supersedes: [ADR-0001](../superseded/ADR-0001-perception-currently-noop.md)
...
These four axes are documented in [ui-essence-layers.md](../../concepts/background/ui-essence-layers.md) Layer 2 (cognitive) and Layer 3 (semantic), drawing on:
...
- Implemented as a no-op placeholder (see `src/closure.c:131` and [ADR-0001](../superseded/ADR-0001-perception-currently-noop.md))
...
2. **The honest-claim requirement**: [path-C-lineage.md](../../concepts/background/path-C-lineage.md) commits Planex to "honest acknowledgment" — no over-claiming. The current "3 abstractions" claim over-claims because the 4th axis is unabstracted.
...
- Backward compatibility is NOT required (Planex is pre-v1.0, see [NG-4](../../concepts/canonical/non-goals.md))
...
`docs/concepts/why-three-abstractions.md` is renamed to `docs/concepts/canonical/why-four-abstractions.md` and rewritten to reflect the four-axis essence mapping.
...
This decision directly addresses an unabstracted essence axis. Layer 3 of [ui-essence-layers.md](../../concepts/background/ui-essence-layers.md) is the layer being completed.
...
This decision closes the gap recorded in [ADR-0001](../superseded/ADR-0001-perception-currently-noop.md). ADR-0001 is superseded.
...
- Existing 25 demos' API breaks (acceptable per [NG-4](../../concepts/canonical/non-goals.md) backward-compatibility pre-v1.0)
...
**Scope statement**: Planex does not target video playback UIs. This is recorded as a non-goal (see [NG-13](../../concepts/canonical/non-goals.md) — to be added).
...
**Scope statement**: This is recorded as [Limitations L11](../../concepts/state/limitations.md) — multi-frame interaction processes are not abstracted, by any UI library including Planex. See [continuous-intent-speculation.md](../../concepts/speculation/continuous-intent-speculation.md) for the future-research marker. (b) route does not address this; it's orthogonal.
...
**Scope statement**: GPU backend is [NG-7](../../concepts/canonical/non-goals.md) for now. When it becomes a real goal, the cost will be addressed via texture pooling or render-cmd abstraction layered on top of Perception — both compatible with (b)'s structure.
...
- **Planex is now explicitly a Layer 1-3 implementation with Layer 4 seed** (per [ui-essence-layers.md](../../concepts/background/ui-essence-layers.md)). No change to which layers are implemented, just sharper alignment within Layer 2-3.
...
- **Why rejected**: Doesn't make a decision. Defers the question while letting both code paths accumulate. Research-grade projects that defer decisions tend to die with both paths half-implemented (see [path-C-lineage.md](../../concepts/background/path-C-lineage.md) Failure mode 5 — confusing research with production).
...
This ADR decides only one thing: that Perception is the 4th first-class abstraction. It does NOT cover:

- Whether the 4 shipping abstractions (Estimate / Perception / Closure / Relation) **close the essence-coverage space** — ADR-0007 (v2 essence revision) explicitly corrects the implicit "this completes the essence coverage" framing that this ADR carries. The 4-axis coverage claim is downgraded from canonical to superseded-by-ADR-0007.
- The v4 essence-rederivation proposal (Interpretant / Perlocution / Breakdown) — these are separate proposals tracked in `essence-derivation-v4-clean.md` and ADR-0010, not promoted by this ADR.
- The `px_loop` 5th abstraction — see ADR-0008 for that decision; this ADR is silent on it.
- Whether the 4 abstractions are *indispensable* — see `abstraction-form.md` Prerequisite 3 (falsifiability) for the three-layer indispensability verification, of which the completeness test (3c) remains an open gap as of v0.5.
- The leak-budget retire curve for Perception — see `leak-budgets.md` and ADR-0013 for the L1/L2 audit that tracks when Perception-level leaks are retired.
...
- 2026-08-25: Accepted
- 2026-08-27: Framing corrected by ADR-0007 (v2 essence revision) — the "completes the essence coverage" claim is downgraded; the decision to promote Perception stands
- 2026-08-27: Implementation extended by ADR-0008 (Feedback as 5th essence category) — `px_loop` joins as the 5th abstraction; the 4-abstraction framing is no longer current
...
- Related ADRs:
  - [ADR-0001](../superseded/ADR-0001-perception-currently-noop.md) — **superseded by this ADR**
