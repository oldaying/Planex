...
`docs/concepts/canonical/abstraction-form.md` Prerequisite 2 (orthogonal separability) honesty-table row states:
...
This is an **L2 leak** by the criterion in `docs/concepts/canonical/leak-budgets.md`: "An operation leaks when the user must understand or interact with an underlying mechanism that the abstraction's denotational semantics does not capture." Here, the *constructor signature* leaks a claim of dependency that the operations don't honor. The user is told (via the type signature) "this interpretant is bound to this perception" — but the binding is documentation-only, not enforced or used.
...
- **Related docs:** [`docs/concepts/canonical/abstraction-form.md`](../../concepts/canonical/abstraction-form.md) — Prerequisite 2 honesty-table row updated by this ADR
- **Related docs:** [`docs/concepts/canonical/leak-budgets.md`](../../concepts/canonical/leak-budgets.md) — v4 preview section added by this ADR
- **Related docs:** [`docs/concepts/history/essence-derivation-v4-clean.md`](../../concepts/history/essence-derivation-v4-clean.md) — postscript added by this ADR
