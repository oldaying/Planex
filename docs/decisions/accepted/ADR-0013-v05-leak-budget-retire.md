...
[`docs/concepts/canonical/leak-budgets.md`](../../concepts/canonical/leak-budgets.md) (commit `76303dc`, v0.4) prescribed a falsifiable retire curve for the L2 (semantic) leak count per abstraction:
...
This ADR records the v0.5 retire cycle only. It does NOT cover:

- Closing the leak budget to zero — 7 L2 leaks were retired in v0.5, dropping shipping L2 from 17% to 3.8%, but 3.8% remains. The next retire cycle (v0.6) is a separate future ADR; this ADR does not commit to its schedule or its target percentage.
- L1 leaks (host-tax / Spolsky-grade) — `leak-budgets.md` distinguishes L1 (mechanism leaks, the host-tax dimension) from L2 (semantic leaks, the abstraction-eroding dimension). This ADR retires only L2 leaks. L1 leaks (e.g., platform-specific includes in cross-platform headers) are tracked separately and are out of scope here.
- The leak-budget verification automation — the ADR's Q4 commits to a verifiable cost scenario (the `make leak-audit` target), but the CI integration that makes leak-audit failures block PR merges is documented but not yet enforced. This is the same Q3 self-acknowledged gap pattern as ADR-0011.
- The completeness-test gap (`abstraction-form.md` Prerequisite 3c) — this retire verifies Layer 3 falsifiability at the API surface but does not close the closed-corpus + bidirectional-coverage gap that would make the indispensability claim fully falsifiable.
- The deprecation registry — the API removed in v4 (`px_closure_get_status`) is the first entry in `docs/reference/deprecation-registry.md`, which is created by the same wave as this ADR. Future retires extend the registry; this ADR records only the v0.5 retire.

## HISTORY

- 2026-08-28: Accepted (immediate — single-maintainer posture, no FCP)

## References

- [`docs/concepts/canonical/leak-budgets.md`](../../concepts/canonical/leak-budgets.md) — the document this ADR implements.
- [`docs/concepts/canonical/abstraction-form.md`](../../concepts/canonical/abstraction-form.md) — Prerequisite 2 (orthogonal separability) and Prerequisite 3 (falsifiability) that this retire verifies.
- [`docs/concepts/state/limitations.md`](../../concepts/state/limitations.md) — L1 (Perception Phase 2 pending), closed by this retire.
