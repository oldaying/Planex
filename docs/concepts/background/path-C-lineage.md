...
- [Why Four Abstractions](../canonical/why-four-abstractions.md) — Planex's manifesto (the synthesis that makes Planex's attempt different from predecessors)
...
- [Limitations L10](../state/limitations.md) — single-maintainer bus factor
- [Non-Goals](../canonical/non-goals.md) — what Planex deliberately does not aspire to
- [Roadmap Matrix](../state/roadmap-matrix.md) — current state of the attempt
