...
For the theoretical foundations of these terms, see [Why Four Abstractions](why-four-abstractions.md) and the ADRs in [../decisions/](../decisions/).
...
**Critical:** Intent is a value, not a function. This is Planex's defining design choice — see [ADR-0001](../decisions/ADR-0001-perception-currently-noop.md) and [ADR-0003](../decisions/ADR-0003-no-ai-integration.md) for consequences.
...
The fifth stage of a Closure. A function (`px_perception_fn`) that returns the visible state after the Action executes. **Currently a no-op placeholder** — see [ADR-0001](../decisions/ADR-0001-perception-currently-noop.md) and [Limitations L1](limitations.md#l1-perception-is-a-no-op-architectural-gap).
...
See [ADR-0002](../decisions/ADR-0002-relation-necessity-pending-undo.md) for the open question of Relation's necessity.
...
- [Why Four Abstractions](why-four-abstractions.md) — manifesto
- [API Reference](../reference/api.md) — function-level documentation
- [Limitations](limitations.md) — where these terms exceed current implementation
