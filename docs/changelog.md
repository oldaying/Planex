...
### Changed — CI: the windows job stops being able to lie (three false-PASS shapes closed; the c098b63 hold-open items land)

- **`/WX` lands** (the MSVC `-Werror` equivalent): the build has been warning-free since c098b63 (the C4244 float literals and the C4996 POSIX-name nags fixed at the source level; the latest windows-latest build log — run 33388320286, VS 2026 / MSVC 19.51 — contains zero `warning C` lines), so a NEW warning from a future toolchain or SDK rev now fails the build instead of scrolling past. This is the MSVC mirror of the `strict-warnings` Linux job, applied to the job that already builds everything.
- **The missing-exe precheck**: a `Test-Path` pass over all ten test executables before anything runs. PowerShell's call operator on a missing path does not fail a step — `$LASTEXITCODE` keeps its stale value — the exact false-PASS shape the local pwsh loop hit during the c098b63 diagnosis (a `CommandNotFoundException` followed by a green PASS printed from the previous command's exit code). A silently-unbuilt binary now fails loudly before the first test.
- **The exit-code loop**: the old one-command-per-line form had the mirror-image flaw — a failing suite mid-list left the step's exit code to whatever the LAST command returned. The loop records every nonzero exit per suite and fails the step on any failure; assertions are live in Release per c098b63's `#undef NDEBUG` guards (Gate 4 of static-analysis.yml keeps them that way).
