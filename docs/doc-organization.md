...
| ~~Write `docs/concepts/canonical/intent.md` (one-page Intent per subsystem)~~ | ~~18~~ | **Landed in commit (T6b).** See [`docs/concepts/canonical/intent.md`](concepts/canonical/intent.md) — one-page orientation sheet per Stone Librande's GDC 2010 "one-page design" format: vision statement (1 sentence), 3 design pillars with falsifiable tests, abstraction table (7 since v0.7), ASCII core-loop diagram, non-goals paired with the pillar each protects, 3 prerequisites + fallback table, 9-gate CI contract table, reading order. Read first; everything else in `canonical/` defends it at length. |
